 ...
 * Followers Page
 * 
 * List of users following a profile
 */
...
export async function generateMetadata({ params }: FollowersPageProps) {
  ...
  return {
    title: `People following ${profile.name || profile.username} | Social`,
  };
}
...
export default async function FollowersPage({ params }: FollowersPageProps) {
  ...
  const { users, nextCursor } = await getFollowers(username);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            @{username} · {profile._count.followers} followers
          </p>
        </div>
      </div>
